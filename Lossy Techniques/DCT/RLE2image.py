...
print("deCompression Time:", compression_time)
# compressed image is written into compressed_image.mp file
cv2.imwrite("compressed_image.bmp",np.uint8(padded_img))
# Load the original image
original_image = cv2.imread('fruits.jpg', cv2.IMREAD_GRAYSCALE)
re = cv2.imread('compressed_image.bmp', cv2.IMREAD_GRAYSCALE)
reconstructed_image = cv2.resize(re, (original_image.shape[1], original_image.shape[0]))
